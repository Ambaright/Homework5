...
```{r}
school_data |>
  summarize(mean_age = mean(age), sd_age = sd(age), mean_mat_absences = mean(absences.x),
            sd_mat_absences = sd(absences.x), mean_por_G3 = mean(G3.y), sd_por_G3 = sd(G3.y))
```
...
Here we will group according to `address`.
...
Again we will group by `address`, and include `Pstatus` as an extra grouping variable.
...
Here we can look at the `age` and `G3.x` (math) across the `address` variable.
